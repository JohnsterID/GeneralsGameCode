...
class PropPageRingGenBase : public wxDialog
{
public:
    PropPageRingGenBase() {}
    PropPageRingGenBase(wxWindow *parent)
    {
        Create(parent);
    }
    ...
    {
        if (!wxXmlResource::Get()->LoadDialog(this, parent, "IDD_PROP_PAGE_RING_GEN")) {
            return false;
        }
        ...
        m_idc_texture_tile_edit = XRCCTRL(*this, "IDC_TEXTURE_TILE_EDIT", wxTextCtrl);
        m_idc_texture_tile_spin = XRCCTRL(*this, "IDC_TEXTURE_TILE_SPIN", wxSpinButton);
...
protected:
    ...
    wxTextCtrl *m_idc_texture_tile_edit;
    wxSpinButton *m_idc_texture_tile_spin;
};

// Derived class for event handlers and custom logic
class PropPageRingGen : public PropPageRingGenBase
{
public:
...
private:
    ...
    void OnChangeTextureTileEdit(wxCommandEvent &event);  // Text control change

    // Spin button handlers (MFC: OnNotify with Update_Spinner_Buddy)
    void OnLifetimeSpin(wxSpinEvent &event);
    void OnTextureTileSpin(wxSpinEvent &event);
