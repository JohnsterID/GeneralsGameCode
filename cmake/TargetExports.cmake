# TheSuperHackers: Target exports module for cross-platform symbol export
...
function(target_exports target)
    ...
    if(NOT SCRIPTS_SYMBOLS)
        message(FATAL_ERROR "No symbols provided for export.")
    elseif(MSVC)
    ...
    elseif(APPLE)
        ...
        # Add underscore prefix to each symbol (Darwin convention)
        set(_symbols "")
        foreach(_symbol ${SCRIPTS_SYMBOLS})
            set(_symbols "${_symbols}_${_symbol}\n")
        endforeach()
        set(_symbols_list "${CMAKE_CURRENT_BINARY_DIR}/${target}symbols.list")
        file(WRITE ${_symbols_list} "${_symbols}")
        target_link_options(${target} PRIVATE
            "-Wl,-exported_symbols_list,${_symbols_list}"
        )
    else()
        message(WARNING "No suitable linker options for your tool chain to export defined symbols.")
    endif()
endfunction()