...
void PropPageEmitterLinegroup::OnInitDialog(wxInitDialogEvent& event)
{
    ...
    //
    // Converted: m_BlurTimeBar = ColorBarClass::Get_Color_Bar (::GetDlgItem (m_hWnd, IDC_BLUR_TIME_BAR));
    //
    // Setup the spinners
    //
    // Converted: Initialize_Spinner (m_BlurTimeRandomSpin, m_BlurTimes.Rand, 0, 10000);
    //
    //	Reset the keyframe control
    //
    // Converted: m_BlurTimeBar->Set_Range (0, 1);
    // Converted: m_BlurTimeBar->Clear_Points ();
    // Converted: m_BlurTimeBar->Modify_Point (0, 0, 0, 0, 0);
    // Converted: m_BlurTimeBar->Set_Graph_Percent (0, Normalize_Blur_Time(m_BlurTimes.Start));
    //
    // Load the current set of frame keyframes into the control
    //
    // Converted: m_BlurTimeBar->Modify_Point (index + 1,
    // Converted: m_BlurTimes.KeyTimes[index] / m_Lifetime,
    // Converted: 0,
    // Converted: 0,
    // Converted: 0);
    // Converted: m_BlurTimeBar->Set_Graph_Percent (index + 1, Normalize_Blur_Time(m_BlurTimes.Values[index]));
    // Converted: return TRUE;  // return TRUE unless you set the focus to a control
